...
def check(masv):
    ...
    sql = 'SELECT * FROM diemdanh WHERE MASV=? AND DATETIME>=? AND DATETIME<=? '
    conn=sqlite3.connect("FaceBase4.db")
    cursor=conn.execute(sql,[masv,Current_Date.strftime("%d-%m-%Y"), NextDay_Date.strftime("%d-%m-%Y")])
